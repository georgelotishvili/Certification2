(function (global) {
  function createMultiFunctionalModule(context) {
    const {
      DOM,
      API_BASE,
      on,
    ...
    async function fetchProjectsFromServer() {
      const response = await fetch(`${API_BASE}/admin/multi-functional/projects`, {
    ...
    async function fetchSettingsFromServer() {
      const response = await fetch(`${API_BASE}/admin/multi-functional/settings`, {
        headers: { ...getAdminHeaders(), ...getActorHeaders() },
      });
    ...
    function populateSettingsFields() {
      ...
      if (DOM.multiFunctionalDurationInput) DOM.multiFunctionalDurationInput.value = durationValue ? String(durationValue) : '';
      if (DOM.multiFunctionalGatePwdInput) DOM.multiFunctionalGatePwdInput.value = state.settings.gatePassword || '';
    }
    ...
    function render() {
      ...
      addTile.type = 'button';
      addTile.id = 'addMultiFunctionalTile';
    ...
    async function persistSettings(patch = {}, { notifyDuration = false, notifyPassword = false } = {}) {
      ...
      try {
        ...
        if (notifyDuration && DOM.multiFunctionalDurationFlash) {
          const value = Number(state.settings.durationMinutes || 0);
          DOM.multiFunctionalDurationFlash.textContent = `ხანგრძლივობა შეიცვალა: ${value} წუთი`;
          DOM.multiFunctionalDurationFlash.style.display = 'block';
          setTimeout(() => {
            DOM.multiFunctionalDurationFlash && (DOM.multiFunctionalDurationFlash.style.display = 'none');
          }, 3000);
        }
        if (notifyPassword) {
          showToast('მრავალფუნქციურის ადმინისტრატორის პაროლი შენახულია');
        }
      } catch (err) {
        showToast('მრავალფუნქციურის პარამეტრების შენახვა ვერ მოხერხდა', 'error');
      }
    }
    ...
    async function loadSettings() {
      try {
        ...
        showToast('მრავალფუნქციურის პარამეტრების ჩატვირთვა ვერ მოხერხდა', 'error');
    ...
    function saveDuration() {
      const value = Number(DOM.multiFunctionalDurationInput?.value || 0);
      if (!value || value < 1) {
    ...
    function saveGatePassword() {
      const value = String(DOM.multiFunctionalGatePwdInput?.value || '').trim();
      if (!value) {
    ...
    function handleGridClick(event) {
      ...
      if (target.closest?.('#addMultiFunctionalTile')) {
        addProject();
        return;
      }
    ...
    async function deleteProject(project) {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE}/admin/multi-functional/projects/${projectIdInt}`, {
          method: 'DELETE',
          headers: { ...getAdminHeaders(), ...getActorHeaders() },
        });
        if (!response.ok) {
    ...
    async function uploadPdf(project, file) {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE}/admin/multi-functional/projects/${projectIdInt}/pdf`, {
          method: 'POST',
          headers: { ...getAdminHeaders(), ...getActorHeaders() },
          body: formData,
        });
    ...
    function init() {
      DOM_ELEMENTS.grid = document.getElementById('multiFunctionalGrid');
      DOM_ELEMENTS.blocksCount = document.getElementById('multiFunctionalBlocksCount');
      if (!DOM_ELEMENTS.grid) return;

      // Wire settings controls
      on(DOM.multiFunctionalDurationSaveBtn, 'click', saveDuration);
      on(DOM.multiFunctionalGatePwdSaveBtn, 'click', saveGatePassword);
      on(DOM.multiFunctionalGatePwdInput, 'input', handleGatePwdInput);
  ...
  if (typeof global !== 'undefined' && global.AdminModules) {
    global.AdminModules.createMultiFunctionalModule = createMultiFunctionalModule;
  }
