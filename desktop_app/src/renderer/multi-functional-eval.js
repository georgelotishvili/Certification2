...
function updateUserDisplay() {
    ...
    // სახელი და გვარი - შეიძლება იყოს full_name ან first_name + last_name
    const fullName = user.full_name || `${user.first_name || ''} ${user.last_name || ''}`.trim();
    if (nameEl) nameEl.textContent = fullName || '---';
    
    // უნიკალური კოდი - შეიძლება იყოს personal_number, code ან personal_id
    const userCode = user.personal_number || user.code || user.personal_id || '---';
...
function setupEventListeners() {
    ...
    if (returnHomeBtn) {
        returnHomeBtn.addEventListener('click', async () => {
            ...
            // გამოცდის უფლების გათიშვა (exam_permission = false)
...
// Exam Permission
// ==========================================
async function revokeExamPermission() {
    try {
        const token = localStorage.getItem('auth_token');
        if (!token) {
            console.log('No auth token - skipping permission revoke');
            return;
        }
        
        const response = await fetch(`${window.API_CONFIG.baseURL}/users/me/revoke-exam-permission`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json',
        ...
        if (response.ok || response.status === 204) {
            console.log('✅ Exam permission revoked successfully');
            
            // განვაახლოთ localStorage-ში შენახული მომხმარებლის ინფორმაცია
            try {
                const userStr = localStorage.getItem('current_user');
                if (userStr) {
            ...
            console.error('Failed to revoke exam permission:', response.status);
        }
    } catch (e) {
        console.error('Error revoking exam permission:', e);
    }
}
